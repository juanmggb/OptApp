...
with col2: 
    if table == "Productos": 
        ...
        st.write(database.select(table="Clientes", attribute=None, column=None, value= None))
    else: 
        pass 
...
with col_1:
 ...
 with st.form(key ='Producto'):
   ...
   Productos = database.select(table='Productos',attribute = None, column = None, value = None)
...
with col_2: 
    ...
    with st.form(key='Empleado'):
        ...
        ID_E = st.selectbox('Selecciona el ID del empleado que realizara la venta',id_e, index=1, key=2)
